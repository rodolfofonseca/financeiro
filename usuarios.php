...
router_add('pesquisa_usuario', function () {
    ...
    if ($codigo_usuario != '') {
        $retorno_usuario = (array) $objeto_usuario->pesquisar((array) ['filtro' => (array) ['where' => [['codigo_usuario', '=', $codigo_usuario]]]]);
    }
...
router_add('pesquisar_usuarios', function () {
    $objeto_usuario = new Usuario();
    $empresa = (int) (isset($_REQUEST['empresa']) ? (int) intval($_REQUEST['empresa']) : 0);
    $tipo_usuario = (string) (isset($_REQUEST['tipo_usuario']) ? (string) $_REQUEST['tipo_usuario'] : 'Administrador');
    $status_usuario = (bool) (isset($_REQUEST['status_usuario']) ? (bool) filter_var($_REQUEST['status_usuario'], FILTER_VALIDATE_BOOLEAN) : true);
    ...
    array_push($filtro_montando, ['status_usuario', '=', (bool) $status_usuario]);

    if (empty($filtro_montando) == false) {
        $retorno = (array) $objeto_usuario->pesquisar_todos((array) ['filtro' => (array) ['where' => (array) $filtro_montando], 'ordenacao' => (array) [['nome_usuario', 'ASC']], 'limite' => (int) 0]);
    }
