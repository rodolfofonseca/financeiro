...
class ContasPagarReceber implements InterfaceModelo
{
    ...
    public function baixar_contas($dados)
    {
        ...
        if (empty($conta) == false) {
            ...
            if ($retorno_update == true) {
                ...
                $dados_movimentacao = (array) ['empresa' => $this->empresa, 'conta' => (int) intval($dados['codigo_conta_bancaria'], 10), 'valor_lancamento' => (float) $this->valor_pago, 'data_lancamento' => (string) $dados['data_baixa']];
    ...
    {
        $retorno = (bool) model_update((string) 'conta_pagar_receber', (array) ['where' => [['codigo_empresa', '=', $dados['codigo_empresa']], ['status_conta', '=', 'AGUARDANDO'], ['data_vencimento', '<=', model_date($dados['data_vencimento'], '23:59:59')]]], (array) ['status_conta' => (string) 'VENCIDA']);
        return (bool) $retorno
;    }
    ...
    public function pesquisar_contas_vencidas($dados)
    {
        ...
        $filtro = ['filtro' => (array) [], 'ordenacao' => (array) [['data_vencimento', 'ASC']], 'limite' => (int) 0];
  ...
  AND data_vencimento BETWEEN :inicio AND :fim
    ...
    /**
     * Função responsável por somar as contas vencidas, mostrando a notificação para o usuário
     * @param mixed $codigo_empresa
     * @return array
     */
    public function somar_conta_vencida($codigo_empresa){
        $query = (string) "SELECT SUM(valor_conta) AS valor_total_contas, COUNT(*) AS quantidade_contas, CASE WHEN tipo_conta THEN 'RECEBER' ELSE 'PAGAR' END AS tipo_conta FROM conta_pagar_receber WHERE codigo_empresa = :empresa AND status_conta = 'VENCIDA' GROUP BY tipo_conta;";

        $dados = (array) model_query($query, (array) ['empresa' => $codigo_empresa]);
