var sistema = (function (window) {
    ...
    var sistema = {
        ...
        gerar_td: (function (classe, texto, tipo = '', colspan = false, valor_colpan = '', title = '') {
        ...
        gerar_a: function (texto, classe, funcao = null, atributos = {}) {
            ...
            for (let contador = 0; contador < classe.length; contador++) {
                a.classList.add(classe[contador]);
            }
        ...
        abrir_modal: (function (largura, altura, url, nome) {
        ...
        gerar_input: (function (id_input, valor, classe, tipo, mascara = '', disabled = true) {
            let input = document.createElement('input');

            for (let contador = 0; contador < classe.length; contador++) {
                input.classList.add(classe[contador]);
            }

            if (mascara == 'moeda') {
                input.dataset('sistema-mask', 'moeda');
            }
        ...
        listeners: {
            ...
            conta_contabil: function (e) {
                ...
                if (value.length > 2) partes.push(value.substr(2, 2));
                if (value.length > 4) partes.push(value.substr(4, 3));
                if (value.length > 7) partes.push(value.substr(7, 4));
                if (value.length > 11) partes.push(value.substr(11, 6));
        ...
        mask: (function () {
            window.document.addEventListener('focus', function (event) {
                ...
                if (element.getAttribute('sistema-mask') == 'codigo') {
                    ...
                    element.removeEventListener('keydown', sistema.listeners.conta_contabil, true);
                    element.addEventListener('keydown', sistema.listeners.conta_contabil, true);
                
                    element.removeEventListener('blur', sistema.validation.conta_contabil, true);
                    element.addEventListener('blur', sistema.validation.conta_contabil, true);
                }
