...
router_add('index', function () {
    ...
    $retorno_inativacao_usuario = (array) $objeto_usuario->inativar_usuario_120_dias((array) ['empresa' => (int) intval($codigo_empresa, 10)]);
    $retorno_alterar_status_conta = (bool) $objeto_contas_pagar_receber->alterar_status((array) ['codigo_empresa' => (int) intval($codigo_empresa, 10), 'data_vencimento' => (string) $data_vencimento]);

    $retorno_conta = (array) $objeto_conta->pesquisar_todos((array) ['filtro' => (array) ['where' => [['codigo_empresa', '=', EMPRESA, 'data_vencimento']]]]);
        ...
        */
        function montar_relatorio_contas() {
            ...
            sistema.each(CONTAS, function (contador, conta) {
                ...
                if (conta.saldo_conta == 0 || conta.saldo_conta == null) {
    ...
    <div class="page-wrapper">
        <div class="content">
            ...
            if ($retorno_contas_vencidas[0]['quantidade_contas'] > 0) {
                $valor_conta_pagar = (float) 0;
                $valor_conta_receber = (float) 0;
                $quantidade_conta_pagar = (int) 0;
                $quantidade_conta_receber = (int) 0;

                foreach ($retorno_contas_vencidas as $conta) {
                    if ($conta['tipo_conta'] == 'PAGAR') {
                        $valor_conta_pagar = (float) $conta['valor_total_contas'];
                        $quantidade_conta_pagar = (int) $conta['quantidade_contas'];
                    } else {
                        $valor_conta_receber = (float) $conta['valor_total_contas'];
                        $quantidade_conta_receber = (int) $conta['quantidade_contas'];
                    }
                }

                if ($quantidade_contas_pagar >= 1 || $quantidade_conta_receber >= 1) {
                    ...
                    <div class="row">
                        <div class="col-lg-8 col-md-9 col-sm-7">
                            <div>
                                <h5 class="text-white mb-1"><?php echo $texto; ?></h5>
                                <?php
                                if ($quantidade_conta_pagar >= 1) {
                                    echo "<p class='text-white mb-3'>Você possui o total de (<strong>".$quantidade_conta_pagar."</strong>) à <strong>PAGAR</strong> vencidas no valor total de R$: <strong>".formatar_numero($valor_conta_pagar, 2, ',', '.')."</strong></p>";
                                }

                                if($quantidade_conta_receber >= 1){
                                    echo "<p class='text-white mb-3'>Você possui o total de (<strong>".$quantidade_conta_receber."</strong>) à <strong>RECEBER</strong> vencidas no valor total de R$: <strong>".formatar_numero($valor_conta_receber, 2, ',', '.')."</strong></p>";
                                }
                                ?>
