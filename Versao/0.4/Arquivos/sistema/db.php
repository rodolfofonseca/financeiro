...
class DB
{
  ...
  {
    $result = $this->client->selectCollection($this->db, 'counters')->findOneAndUpdate(['_id' => $name], ['$inc' => ['seq' => 1]], ['upsert' => true, 'returnDocument' => MongoDB\Operation\FindOneAndUpdate::RETURN_DOCUMENT_AFTER]);

    return (int) $result['seq'];
  }

  /**
   * Função responsável por realizar o cadastro de um novo item no banco de dados retornando o seu código sequencial e o _id.
   * @param mixed $table
   * @param mixed $field_sequence
   * @param mixed $data
   * @return array{_id: string, sequence: mixed, success: bool|bool}
   */
  static function insert_sequence($table, $field_sequence, $data)
  {
    $sequence_name = $table . '_' . $field_sequence;

    $data[$field_sequence] = DB::use($table)->sequence($sequence_name);

    $result = DB::use($table)->insert($data);

    if ($result === false) {
      return (array) ['success' => false, 'sequence' => '', '_id' => (string) ''];
    }

    return (array) ['success' => true, 'sequence' => $data[$field_sequence], '_id' => (string) $result];
  }
}
?>