...
/** 
 * Rota responsável por pesquisar o produto e retorna o estoque do produto, além do fornecedor do mesmo.
 */
router_add('pesquisar_produto_estoque', function () {
    $objeto_produto = new Produtos();

    echo json_encode($objeto_produto->pesquisar_produtos_estoque($_REQUEST), JSON_UNESCAPED_UNICODE);
});
...
/** 
 * Rota responsável por fazer a falidação do código de barras do produto
 */
router_add('validar_codigo_barras', function () {
    $objeto_ean = new EAN13();

    $codigo_barras_a_validar = (string) (isset($_REQUEST['codigo_barras']) ? (string) $_REQUEST['codigo_barras'] : '');
    $codigo_barras = (string) '';

    if ($codigo_barras_a_validar != '') {
        $codigo_barras = $objeto_ean->getFullCode($codigo_barras_a_validar);
    }

    echo json_encode(['codigo_barras' => (string) $codigo_barras], JSON_UNESCAPED_UNICODE);
    exit;
});
...
router_add('index', function () {
    ...
    <script>
        ...
        const CNPJ_EMPRESA = "<?php echo $_SESSION['cnpj_empresa']; ?>";
        ...
        function pesquisar_produtos() {
            ...
            dados = adicionar(dados, 'nome_produto', nome_produto);
            dados = adicionar(dados, 'status_produto', status_produto);
            dados = adicionar(dados, 'tipo_produto', tipo_produto);
            dados = adicionar(dados, 'unidade_medida', unidade_medida);
            dados = adicionar(dados, 'data_cadastro', data_cadastro);

            barra_progresso('Carregando produtos...');
            ...
            sistema.request.post('/produtos.php', dados, function (retorno) {
                ...
                if (tamanho_retorno == 0) {
                    ...
                    Swal.fire({ icon: 'warning', title: 'Nenhum produto encontrado!' });
                    return;
                } else {
                    function processar_item() {
                        ...
                        a_text.textContent = produto.sku_produto;
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-1 text-center">
                                    ...
                                    <select class="form-control select2" id="status_produto">
                                ...
                                <div class="col-1 text-center">
                                    ...
                                    <select class="form-control select2" id="tipo_produto">
                                ...
                                <div class="col-2 text-center">
                                    ...
                                    <select class="form-control select2" id="unidade_medida">
                                        <option value="">Selecione uma opção</option>
...
router_add('cadastro_produtos', function () {
    ...
    ?>
        ...
        <script>
            ...
            let QUANTIDADE_PRODUTOS = <?php echo $quantidade_produtos; ?>;
            ...
            function salvar_dados() {
                ...
                let objeto_sku_produto = document.querySelector('#sku_produto');
                let objeto_nome_produto = document.querySelector('#nome_produto');
                let objeto_fornecedor = document.querySelector('#fornecedor');
                let objeto_codigo_barras = document.querySelector('#codigo_barras');
                let objeto_unidade_medida = document.querySelector('#unidade_medida');
                let objeto_status_produto = document.querySelector('#status_produto');
                let objeto_tipo_produto = document.querySelector('#tipo_produto');
                let objeto_data_cadastro = document.querySelector('#data_cadastro');
                let objeto_valor_venda = document.querySelector('#valor_venda');
                let objeto_quantidade_alerta = document.querySelector('#quantidade_alerta');
                let objeto_valor_custo = document.querySelector('#valor_custo');

                if (objeto_sku_produto.value == '') {
                    validacao = false;
                    validar_campo(objeto_sku_produto, '', false, 'SKU do produto obrigatório!', false);
                    return;
                } else {
                    validar_campo(objeto_sku_produto, '', true, '', false);
                    sku_produto = objeto_sku_produto.value;
                }

                if (objeto_nome_produto.value == '') {
                    validacao = false;
                    validar_campo(objeto_nome_produto, '', false, 'NOME do produto obrigatório!', false);
                    return;
                } else {
                    validar_campo(objeto_nome_produto, '', true, '', false);
                    nome_produto = objeto_nome_produto.value;
                }

                if (objeto_fornecedor.value == '') {
                    validacao = false;
                    validar_campo(objeto_fornecedor, '', false, 'FORNECEDOR do produto é obrigatório!', false);
                    return;
                } else {
                    validar_campo(objeto_fornecedor, '', true, '', false);
                    fornecedor = objeto_fornecedor.value;
                }

                if (objeto_valor_venda.value == '' || objeto_valor_venda.value == 0) {
                    validacao = false;
                    validar_campo(objeto_valor_venda, '', false, 'VALOR DE VENDA do produto é obrigatório!', false);
                    return;
                } else {
                    validar_campo(objeto_valor_venda, '', true, '', false);
                    valor_venda = objeto_valor_venda.value;
                }

                if (objeto_valor_custo.value == '' || objeto_valor_custo.value == 0) {
                    validacao = false;
                    validar_campo(objeto_valor_custo, '', false, 'VALOR DE CUSTO do produto é obrigatório!', false);
                    return;
                } else {
                    validar_campo(objeto_valor_custo, '', true, '', false);
                    valor_custo = objeto_valor_custo.value;
                }

                if (objeto_status_produto.value == '') {
                    validacao = false;
                    validar_campo(objeto_status_produto, '', false, 'STATUS do produto é obrigatório!', false);
                    return;
                } else {
                    validar_campo(objeto_status_produto, '', true, '', false);
                    status_produto = objeto_status_produto.value;
                }

                if (objeto_tipo_produto.value == '') {
                    validacao = false;
                    validar_campo(objeto_tipo_produto, '', false, 'TIPO de produto é obrigatório!', false);
                } else {
                    validar_campo(objeto_tipo_produto, '', true, '', false);
                    tipo_produto = objeto_tipo_produto.value;
                }

                if (objeto_codigo_barras.value == '') {
                    validacao = false;
                    validar_campo(objeto_codigo_barras, '', false, 'CÓDIGO de barras é obrigatório!', false);
                } else {
                    validar_campo(objeto_codigo_barras, '', true, '', false);
                    codigo_barras = objeto_codigo_barras.value;
                }

                descricao = tinymce.get('editor').getContent();
                data_cadastro = objeto_data_cadastro.value;
            ...
            function pesquisar_produto() {
                sistema.request.post('/produtos.php', {
                    ...
                    if (produto.hasOwnProperty('sku_produto') == true) {
            ...
            /** 
             * Função responsável por gerar um código de barras com o próximo número de produto disponívels
            */
            function gerar_codigo_barras() {
                let comeco_cnpj = sistema.cortar_string(CNPJ_EMPRESA.replace(/\D/g, ''), 8, false);
                let quantidade = String(QUANTIDADE_PRODUTOS + 1);
                let zeros = '0'.repeat(12 - comeco_cnpj.length - quantidade.length);
                let codigo_barras = comeco_cnpj + zeros + quantidade;

                sistema.request.post('/produtos.php', { 'rota': 'validar_codigo_barras', 'codigo_barras': codigo_barras }, function (retorno) {
                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-3 text-center">
                                        <label class="text">SKU</label>
                                        <input type="text" class="form-control" id="sku_produto" placeholder="SKU PRODUTO">
                                    </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="position-relative">
                                                ...
                                                <button type="button"
                                                    ...
                                                    onclick="gerar_codigo_barras();">Gerar</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
