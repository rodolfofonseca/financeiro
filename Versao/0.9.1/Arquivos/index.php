...
router_add('login_usuario', function () {
    ...
    if (empty($usuario) == false) {
        ...
        if ($versao_sistema == 'alfa 0.9.1') {
            header('location:dashboard.php');
        } else {
            header('location:index.php?versao=true');
        }
    } else {
...
router_add('index', function () {
    ...
    $versao = (bool) (isset($_GET['versao']) ? (bool) filter_var($_GET['versao'], FILTER_VALIDATE_BOOLEAN):false);
    $senha = (bool) (isset($_GET['senha']) ? (bool) filter_var($_GET['senha'], FILTER_VALIDATE_BOOLEAN):false);
    ?>
    <!DOCTYPE html>
    <html lang="pt-br">
    ...
    <body class="bg-white">
        ...
        <script>
            ...
            const SENHA = "<?php echo $senha ?>";
            const VERSAO = "<?php echo $versao ?>";

            if(VERSAO == '1'){
                Swal.fire({'icon':'warning', 'title':'Sistema Atualizado', 'text':'O Sistema foi atualizado, por favor atualize seus arquivos'});
            }

            if(SENHA == '1'){
                Swal.fire({'icon':'danger', 'title':'Login inválido', 'text':'Login ou senha incorretos'});
            }
        </script>
    </body>
