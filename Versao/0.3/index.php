...
router_add('index', function () {
  ...
  <body>
    ...
    $atualizacao->exibir_mensagem('Desativando a validação do banco de dados!');
    $atualizacao->ativar_desativar_validacoes('contas_pagar_receber', 'off');

    $atualizacao->exibir_mensagem('Desativando a validação da tabela documentos_comprovantes');
    $atualizacao->ativar_desativar_validacoes('documentos_comprovantes', 'off');

    $atualizacao->exibir_mensagem('Desativando a validação do banco de dados da tabela movimentação');
    $atualizacao->ativar_desativar_validacoes('movimentacao', 'off');

    $atualizacao->exibir_mensagem('Desativando a validação do banco de dados da tabela sistema');
    $atualizacao->ativar_desativar_validacoes('movimentacao', 'off');

    $atualizacao->exibir_mensagem('Alterando a validação do banco de dados');
    $atualizacao->alterar_validacao('contas_pagar_receber', (array) ['empresa' => 'objectId', 'cliente_fornecedor' => 'objectId', 'conta_fornecedor' => 'objectId', 'nome_conta' => (string) '', 'descricao' => (string) '', 'valor_conta' => (double) 0, 'valor_pago' => (double) 0, 'valor_juro_desconto' => (double) 0, 'tipo_juro_desconto' => (string) '', 'tipo_conta' => (string) 'PARGAR', 'data_cadastro' => 'date', 'data_vencimento' => 'date', 'data_baixa' => 'date', 'status_conta' => (string) 'AGUARDANDO', 'comprovante' => (string) 'NAO', 'boleto' => (string) 'NAO']);

    $atualizacao->exibir_mensagem('Alterando a validação do documentos_comprovantes');
    $atualizacao->alterar_validacao('documentos_comprovantes', (array) ['empresa' => 'objectId', 'codigo_local' => 'objectId', 'local_documento' => (string) '', 'nome_arquivo' => (string) '', 'data_cadastro' => 'date']);

    $atualizacao->exibir_mensagem('Alterando a validação da tabela movimentação');
    $atualizacao->alterar_validacao('movimentacao', (array) ['conta' => 'objectId', 'empresa' => 'objectId', 'tipo_lancamento' => (string) '', 'valor_lancamento' => (double) 0, 'data_lancamento' => 'date', 'descricao' => (string) '', 'comprovante' => 'bool']);

    $retorno_sistema = (array) model_all((string) 'sistema');

    $atualizacao->exibir_mensagem('Alterando a validação da tabela sistema');
    $atualizacao->alterar_validacao('sistema', (array) [['empresa' => 'objectId', 'versao_sistema' => (string) '', 'anexa_documentos' => (string) '', 'modulo_contabil' => 'bool', 'pedidos' => 'bool']]);

    $atualizacao->exibir_mensagem('Ativando a validação do banco de dados!');
    $atualizacao->ativar_desativar_validacoes('contas_pagar_receber', 'strict');

    $atualizacao->exibir_mensagem('Ativando a validação da tabela documentos_comprovantes');
    $atualizacao->ativar_desativar_validacoes('documentos_comprovantes', 'strict');

    $atualizacao->exibir_mensagem('Ativando a validação do banco de dados da tabela movimentação');
    $atualizacao->ativar_desativar_validacoes('movimentacao', 'strict');

    $atualizacao->exibir_mensagem('Ativando a validação do banco de dados da tabela sistema');
    $atualizacao->ativar_desativar_validacoes('sistema', 'strict');

    $atualizacao->exibir_mensagem('Criando a tabela produtos no banco de dados');
    $atualizacao->criar_tabela_banco_dados('produtos', (array) ['empresa' => 'objectId', 'fornecedor' => 'objectId', 'nome_produto' => (string) '', 'descricao' => (string) '', 'imagem' => (string) '', 'codigo_barras' => (string) '', 'quantidade_alerta' => (float) 0, 'data_cadastro' => 'date', 'valor_venda' => (float) 0, 'valor_custo' => (float) 0, 'unidade_medida' => (string) '', 'status' => 'bool', 'tipo_produto' => 'bool']);
    $atualizacao->exibir_mensagem('Tabela produtos criada com sucesso!');

    $atualizacao->exibir_mensagem('Criando a tabela pedidos no banco de dados');
    $atualizacao->criar_tabela_banco_dados('pedidos', (array) ['empresa' => 'objectId', 'fornecedor' => 'objectId', 'quantidade_total_itens' => (float) 0, 'valor_unitario' => (float) 0, 'valor_unitario' => (float) 0, 'valor_bruto' => (float) 0, 'valor_desconto' => (float) 0, 'valor_frete' => (float) 0, 'valor_liquido' => (float) 0, 'data_cadastro' => 'date', 'data_movimentacao' => 'date', 'status' => (string) '', 'tipo_pedido' => 'bool']);
    $atualizacao->exibir_mensagem('Tabela pedidos criada com sucesso!');

    $atualizacao->exibir_mensagem('Criando a tabela itens_pedidos no banco de dados');
    $atualizacao->criar_tabela_banco_dados('produtos', (array) ['empresa' => 'objectId', 'pedido' => 'objectId', 'produto' => 'objectId', 'quantidade' => (double) 0, 'valor_unitario' => (double) 0, 'valor_bruto' => (double) 0, 'valor_desconto' => (double) 0, 'valor_frete' => (double) 0, 'valor_liquido' => (double) 0, 'tipo_item_pedido' => (string) '', 'data_cadastro' => 'date']);
    $atualizacao->exibir_mensagem('Tabela itens_pedidos criada com sucesso!');

    $atualizacao->exibir_mensagem('Criando a tabela custo_medio no banco de dados');
    $atualizacao->criar_tabela_banco_dados('produtos', (array) ['empresa' => 'objectId', 'pedido' => 'objectId', 'item_pedido' => 'objectId', 'produto' => 'objectId', 'data_custo' => 'date', 'valor_custo' => (double) 0, 'ignorar' => 'bool']);
    $atualizacao->exibir_mensagem('Tabela custo_medio criada com sucesso!');

    $atualizacao->exibir_mensagem('Criando a tabela conta_contabil no banco de dados');
    $atualizacao->criar_tabela_banco_dados('conta_contabil', (array) ['empresa' => 'objectId', 'pedido' => 'objectId', 'item_pedido' => 'objectId', 'produto' => 'objectId', 'data_custo' => 'date', 'valor_custo' => (double) 0, 'ignorar' => 'bool']);
    $atualizacao->exibir_mensagem('Tabela conta_contabil criada com sucesso!');

    $atualizacao->exibir_mensagem('Criando a tabela contas_fornecedores no banco de dados');
    $atualizacao->criar_tabela_banco_dados('conta_contabil', (array) ['empresa' => 'objectId', 'fornecedor' => 'objectId', 'nome_conta' => (string) '', 'descricao' => (string) '', 'status_conta' => 'bool', 'data_cadastro' => 'date']);
    $atualizacao->exibir_mensagem('Tabela contas_fornecedores criada com sucesso!');

    if (empty($retorno_sistema) == false) {
      foreach ($retorno_sistema as $sistema) {
        $sistema['codigo_sistema'] = (string) $sistema['_id'];
        $sistema['versao_sistema'] = (string) 'alfa 0.3';

        model_update((string) 'sistema', ['_id', '===', $sistema['_id']], $sistema);
      }
    }
